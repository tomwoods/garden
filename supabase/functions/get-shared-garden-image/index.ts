...
Deno.serve(async (req: Request) => {
  ...
  try {
    ...
    const { sharedGardenId, plantId, size, signature, timestamp } = body;

    if (!sharedGardenId || !plantId || !size || !signature || !timestamp) {
      return new Response(
    ...
    if (size !== "small" && size !== "large") {
      return new Response(
        JSON.stringify({ error: "size must be 'small' or 'large'" }),
        { status: 400, headers: { ...corsHeaders, "Content-Type": "application/json" } }
      );
    }
    ...
    const message = `fetch-shared:${plantId}:${timestamp}`;
    ...
    const column =
      size === "large" ? "image_data_large" : "image_data_small";

    const { data: imageRow, error: imageError } = await supabase
      .from("shared_garden_images")
