...
Deno.serve(async (req: Request) => {
  ...
  try {
    ...
    const { sharedGardenId, plotActivityId, imageIndex, signature, timestamp } = body;

    if (!sharedGardenId || !plotActivityId || imageIndex === undefined || !signature || !timestamp) {
      return new Response(
    ...
    const message = `delete-plot-activity:${plotActivityId}:${timestamp}`;
