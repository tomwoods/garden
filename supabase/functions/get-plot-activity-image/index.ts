...
Deno.serve(async (req: Request) => {
  ...
  try {
    ...
    const { sharedGardenId, plotActivityId, size, signature, timestamp } = body;

    if (!sharedGardenId || !plotActivityId || !signature || !timestamp) {
      return new Response(
    ...
    const message = `fetch-plot-activity:${plotActivityId}:${timestamp}`;
    ...
    const { data: images, error: fetchErr } = await supabase
      ...
      .select(`image_index, ${column}`)
      .eq("shared_garden_id", sharedGardenId)
      .eq("plot_activity_id", plotActivityId)
    ...
    const result = images.map((img: Record<string, unknown>) => ({
      imageIndex: img.image_index,
      encryptedData: img[column],
    }));

    return new Response(
      JSON.stringify({ success: true, images: result }),
      { headers: { ...corsHeaders, "Content-Type": "application/json" } }
    );
  } catch (err) {
    return new Response(
